...
The map below shows completed and planned deployments for 2026, coloured by institution. Dots with a buoy symbol are locations of completed deployments (deployment location, not most recent profile location), while dots with the loading symbol are planned deployments.
...
df = df.loc[(df['DEPLOYMENT DATE'] > pd.Timestamp('01-01-2026')) | (df['DEPLOYMENT DATE'].isna())]
...
if n_floats == 0:
  summary_string = 'So far in 2026 we have not deployed any floats.'
else:
  summary_string = f'To date we have deployed {n_floats} floats in 2026: '
