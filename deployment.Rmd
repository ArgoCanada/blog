...
plan_table = recent.drop('MODEL_DETAIL', axis=1)
```
...
assign_country <- function(name) {
  ...
  paste(emoji::emoji_name[paste("flag_",cty)], name)
}
py$recent$Ship <- lapply(py$recent$Ship, assign_country)
knitr::kable(py$recent_table)
```
## Planned Deployments

```{r table_2, layout="l-page-outset shaded"}
py$plan$Ship <- lapply(py$plan$Ship, assign_country)
