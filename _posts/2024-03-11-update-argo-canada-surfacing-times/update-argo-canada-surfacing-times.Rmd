...
For each float type, different methods can be used depending on the float mission parameter set to vary the surfacing time of day. Argo Canada currently operates [x] kinds of floats: MetOcean NOVA, NKE ARVOR (SBE and RBR), NKE PROVOR (CTS4 and CTS5 - different operating systems), and NKE deep ARVOR. 
...
- NKE PROVOR CTS5: similar to the ARVORs, a cycle time can be defined. At this stage, Canada has two CTS5 floats deployed by Dalhousie University in the Labrador Sea, operating on 125 hour (5 day plus 5 hour) cycles times. 
